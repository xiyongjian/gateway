...
from gateway.lib.adjusted_array import ensure_adjusted_array, ensure_ndarray
...
from gateway.utils.pandas_utils import explode

from .term import AssetExists, InputDates, LoadableTerm

from gateway.utils.date_utils import compute_date_range_chunks
from gateway.utils.pandas_utils import categorical_df_concat
from gateway.utils.sharedoc import copydoc
...
class PipelineEngine(with_metaclass(ABCMeta)):
    ...
    def run_pipeline(self, pipeline, start_date, end_date):
        ...
        Parameters
        ----------
        pipeline : gateway.pipeline.Pipeline
            The pipeline to run.
        ...
        result : pd.DataFrame
            ...
            instances of :class:`gateway.pipeline.term.Term`.
...
class SimplePipelineEngine(PipelineEngine):
    ...
    populate_initial_workspace : callable, optional
        ...
        :func:`gateway.pipeline.engine.default_populate_initial_workspace`
        for more info.
    ...
    def run_pipeline(self, pipeline, start_date, end_date):
        ...
        --------
        :meth:`gateway.pipeline.engine.PipelineEngine.run_pipeline`
        :meth:`gateway.pipeline.engine.PipelineEngine.run_chunked_pipeline`
        """
    ...
    def compute_chunk(self, graph, dates, assets, initial_workspace):
        ...
        Parameters
        ----------
        graph : gateway.pipeline.graph.TermGraph
        dates : pd.DatetimeIndex
