...
from gateway.assets import Asset
from gateway.errors import IncompatibleTerms
from gateway.pipeline.factors import CustomFactor
from gateway.pipeline.filters import SingleAsset
from gateway.pipeline.mixins import SingleInputMixin, StandardOutputs
from gateway.pipeline.sentinels import NotSpecified
from gateway.pipeline.term import AssetExists
from gateway.utils.input_validation import (
    expect_bounded,
    expect_dtypes,
    expect_types,
)
from gateway.utils.math_utils import nanmean
...
class RollingPearson(_RollingCorrelation):
    ...
    Parameters
    ----------
    base_factor : gateway.pipeline.factors.Factor
...
class RollingLinearRegression(CustomFactor, SingleInputMixin):
    ...
    Parameters
    ----------
    dependent : gateway.pipeline.factors.Factor
        The factor whose columns are the predicted/dependent variable of each
        regression with `independent`.
    independent : gateway.pipeline.slice.Slice or gateway.pipeline.Factor
...
class RollingLinearRegressionOfReturns(RollingLinearRegression):
    ...
    :class:`gateway.pipeline.factors.CustomFactor`.
