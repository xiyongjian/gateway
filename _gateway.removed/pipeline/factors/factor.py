...
from gateway.utils.compat import wraps
from gateway.errors import (
    BadPercentileBounds,
    UnknownRankMethod,
    UnsupportedDataType,
)
from gateway.lib.normalize import naive_grouped_rowwise_apply
from gateway.lib.rank import masked_rankdata_2d, rankdata_1d_descending
from gateway.pipeline.api_utils import restrict_to_dtype
from gateway.pipeline.classifiers import Classifier, Everything, Quantiles
from gateway.pipeline.dtypes import (
    CLASSIFIER_DTYPES,
    FACTOR_DTYPES,
    FILTER_DTYPES,
)
from gateway.pipeline.expression import (
    BadBinaryOperator,
    COMPARISONS,
    is_comparison,
    MATH_BINOPS,
...
from gateway.pipeline.filters import (
    Filter,
    NumExprFilter,
    PercentileFilter,
    MaximumFilter,
...
from gateway.pipeline.mixins import (
    AliasedMixin,
    CustomTermMixin,
    DownsampledMixin,
    LatestMixin,
...
from gateway.pipeline.sentinels import NotSpecified, NotSpecifiedType
from gateway.pipeline.term import ComputableTerm, Term
from gateway.utils.functional import with_doc, with_name
from gateway.utils.input_validation import expect_types
from gateway.utils.math_utils import nanmean, nanstd
from gateway.utils.memoize import classlazyval
from gateway.utils.numpy_utils import (
    bool_dtype,
    coerce_to_dtype,
    float64_dtype,
)
...
class Factor(RestrictedDTypeMixin, ComputableTerm):
    ...
    Factors can also be converted into :class:`gateway.pipeline.Filter` objects
    ...
    def demean(self, mask=NotSpecified, groupby=NotSpecified):
        ...
        Parameters
        ----------
        mask : gateway.pipeline.Filter, optional
            A Filter defining values to ignore when computing means.
        groupby : gateway.pipeline.Classifier, optional
    ...
    def zscore(self, mask=NotSpecified, groupby=NotSpecified):
        ...
        Returns
        -------
        zscored : gateway.pipeline.Factor
        ...
        Examples
        --------
        See :meth:`~gateway.pipeline.factors.Factor.demean` for an in-depth
    ...
    def rank(self,
        ...
        Returns
        -------
        ranks : gateway.pipeline.factors.Rank
        ...
        :class:`gateway.pipeline.factors.factor.Rank`
        """
    ...
    def pearsonr(self, target, correlation_length, mask=NotSpecified):
        ...
        Parameters
        ----------
        target : gateway.pipeline.Term with a numeric dtype
        ...
        Returns
        -------
        correlations : gateway.pipeline.factors.RollingPearson
        ...
        :class:`gateway.pipeline.factors.RollingPearsonOfReturns`
    ...
    def spearmanr(self, target, correlation_length, mask=NotSpecified):
        ...
        Returns
        -------
        correlations : gateway.pipeline.factors.RollingSpearman
            A new Factor that will compute correlations between `target` and
            the columns of `self`.
        ...
        :class:`gateway.pipeline.factors.RollingSpearmanOfReturns`
    ...
    def linear_regression(self, target, regression_length, mask=NotSpecified):
        ...
        Returns
        -------
        regressions : gateway.pipeline.factors.RollingLinearRegression
        ...
        :class:`gateway.pipeline.factors.RollingLinearRegressionOfReturns`
        """
    ...
    def winsorize(self,
        ...
        Returns
        -------
        winsorized : gateway.pipeline.Factor
    ...
    def quantiles(self, bins, mask=NotSpecified):
        ...
        Returns
        -------
        quantiles : gateway.pipeline.classifiers.Quantiles
    ...
    def quartiles(self, mask=NotSpecified):
        ...
        Returns
        -------
        quartiles : gateway.pipeline.classifiers.Quantiles
    ...
    def quintiles(self, mask=NotSpecified):
        ...
        Returns
        -------
        quintiles : gateway.pipeline.classifiers.Quantiles
    ...
    def deciles(self, mask=NotSpecified):
        ...
        Returns
        -------
        deciles : gateway.pipeline.classifiers.Quantiles
    ...
    def top(self, N, mask=NotSpecified, groupby=NotSpecified):
        ...
        Returns
        -------
        filter : gateway.pipeline.filters.Filter
        """
        if N == 1:
    ...
    def bottom(self, N, mask=NotSpecified, groupby=NotSpecified):
        ...
        Returns
        -------
        filter : gateway.pipeline.Filter
        """
    ...
    def percentile_between(self,
        ...
        Returns
        -------
        out : gateway.pipeline.filters.PercentileFilter
            A new filter that will compute the specified percentile-range mask.

        See Also
        --------
        gateway.pipeline.filters.filter.PercentileFilter
        """
    ...
    def isnan(self):
        ...
        Returns
        -------
        nanfilter : gateway.pipeline.filters.Filter
        """
        return self != self
...
class GroupedRowTransform(Factor):
    ...
    See Also
    --------
    gateway.pipeline.factors.Factor.zscore
    gateway.pipeline.factors.Factor.demean
    gateway.pipeline.factors.Factor.rank
    """
    window_length = 0
...
class Rank(SingleInputMixin, Factor):
    ...
    Parameters
    ----------
    factor : gateway.pipeline.factors.Factor
