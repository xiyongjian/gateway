...
from gateway.data.bundles.core import load
...
def load_extensions(default, extensions, strict, environ, reload=False):
    ...
    ----------
    default : bool
        Load the default exension (~/.gateway/extension.py)?
    ...
    for ext in extensions:
        if ext in _loaded_extensions and not reload:
            continue
        try:
            # load all of the gateway extensionss
            if ext.endswith('.py'):
...
def run_algorithm(start,
    ...
    default_extension : bool, optional
        Should the default gateway extension be loaded. This is found at
        ``$GATEWAY_ROOT/extension.py``
    ...
    See Also
    --------
    gateway.data.bundles.bundles : The available data bundles.
    """
