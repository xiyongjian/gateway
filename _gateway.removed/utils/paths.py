"""
Canonical path locations for gateway data.

Paths are rooted at $GATEWAY_ROOT if that environment variable is set.
Otherwise default to expanduser(~/.gateway)
"""
...
def gateway_root(environ=None):
    """
    Get the root directory for all gateway-managed files.
    ...
    -------
    root : string
        Path to the gateway root dir.
    """
    if environ is None:
        environ = os.environ

    root = environ.get('GATEWAY_ROOT', None)
    if root is None:
        root = expanduser('~/.gateway')

    return root


def gateway_path(paths, environ=None):
    """
    Get a path relative to the gateway root.
    ...
    environ : dict, optional
        An environment dict to forward to gateway_root.

    Returns
    -------
    newpath : str
        The requested path joined with the gateway root.
    """
    return join(gateway_root(environ=environ), *paths)


def default_extension(environ=None):
    """
    Get the path to the default gateway extension file.

    Parameters
    ----------
    environ : dict, optional
        An environment dict to forwart to gateway_root.

    Returns
    -------
    default_extension_path : str
        The file path to the default gateway extension file.
    """
    return gateway_path(['extension.py'], environ=environ)


def data_root(environ=None):
    """
    The root directory for gateway data files.
    ...
    """
    return gateway_path(['data'], environ=environ)
...
def data_path(paths, environ=None):
    """
    Get a path relative to the gateway data directory.
    ...
    -------
    newpath : str
        The requested path joined with the gateway data root.
    """
    return gateway_path(['data'] + list(paths), environ=environ)


def cache_root(environ=None):
    """
    The root directory for gateway cache files.
    ...
    -------
    cache_root : str
       The gateway cache root.
    """
    return gateway_path(['cache'], environ=environ)
...
def cache_path(paths, environ=None):
    """
    Get a path relative to the gateway cache directory.
    ...
    -------
    newpath : str
        The requested path joined with the gateway cache root.
    """
    return gateway_path(['cache'] + list(paths), environ=environ)
