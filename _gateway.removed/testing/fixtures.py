...
import gateway
from gateway.assets import Equity, Future
from gateway.finance.asset_restrictions import NoRestrictions
from gateway.pipeline import SimplePipelineEngine
from gateway.pipeline.data import USEquityPricing
from gateway.pipeline.loaders import USEquityPricingLoader
from gateway.pipeline.loaders.testing import make_seeded_random_loader
from gateway.protocol import BarData
from gateway.utils.calendars import (
    get_calendar,
    register_calendar)
from gateway.utils.paths import ensure_directory

gateway_dir = os.path.dirname(gateway.__file__)


class GatewayTestCase(with_metaclass(FinalMeta, TestCase)):
    """
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            assert cls._base_init_fixtures_was_called, (
                "GatewayTestCase.init_class_fixtures() was not called.\n"
    ...
    def setUp(self):
        ...
        try:
            ...
            assert self._init_instance_fixtures_was_called, (
                "GatewayTestCase.init_instance_fixtures() was not"
                " called.\n"
...
class WithDefaultDateBounds(object):
    """
    GatewayTestCase mixin which makes it possible to synchronize date bounds
    across fixtures.
...
class WithLogger(object):
    """
    GatewayTestCase mixin providing cls.log_handler as an instance-level
    fixture.
...
class WithAssetFinder(WithDefaultDateBounds):
    """
    GatewayTestCase mixin providing cls.asset_finder as a class-level fixture.
    ...
    See Also
    --------
    gateway.testing.make_simple_equity_info
    gateway.testing.make_jagged_equity_info
    gateway.testing.make_rotating_equity_info
    gateway.testing.make_future_info
    gateway.testing.make_commodity_future_info
    """
    ...
    def make_asset_finder(cls):
        ...
        Returns
        -------
        asset_finder : gateway.assets.AssetFinder
        """
...
class WithTradingCalendars(object):
    """
    GatewayTestCase mixin providing cls.trading_calendar,
...
class WithTradingEnvironment(WithAssetFinder,
    ...
    """
    GatewayTestCase mixin providing cls.env as a class-level fixture.
    ...
    See Also
    --------
    :class:`gateway.finance.trading.TradingEnvironment`
    """
    TRADING_ENV_FUTURE_CHAIN_PREDICATES = None
    MARKET_DATA_DIR = os.path.join(gateway_dir, 'resources', 'market_data')
    ...
    def make_load_function(cls):
        def load(*args, **kwargs):
            ...
            # and treasury curves data, but when running the gateway tests this
...
class WithSimParams(WithTradingEnvironment):
    """
    GatewayTestCase mixin providing cls.sim_params as a class level fixture.
    ...
    See Also
    --------
    gateway.utils.factory.create_simulation_parameters
    """
...
class WithTradingSessions(WithTradingCalendars, WithDefaultDateBounds):
    """
    GatewayTestCase mixin providing cls.trading_days, cls.all_trading_sessions
...
class WithTmpDir(object):
    """
    GatewayTestCase mixing providing cls.tmpdir as a class-level fixture.
...
class WithInstanceTmpDir(object):
    """
    GatewayTestCase mixing providing self.tmpdir as an instance-level fixture.
...
class WithEquityDailyBarData(WithTradingEnvironment):
    """
    GatewayTestCase mixin providing cls.make_equity_daily_bar_data.
    ...
    make_equity_daily_bar_data() -> iterable[(int, pd.DataFrame)]
        ...
        :func:`~gateway.testing.create_daily_bar_data`

    See Also
    --------
    WithEquityMinuteBarData
    gateway.testing.create_daily_bar_data
    """
...
class WithFutureDailyBarData(WithTradingEnvironment):
    """
    GatewayTestCase mixin providing cls.make_future_daily_bar_data.
...
class WithBcolzEquityDailyBarReader(WithEquityDailyBarData, WithTmpDir):
    """
    GatewayTestCase mixin providing cls.bcolz_daily_bar_path,
    ...
    - `cls.bcolz_daily_bar_ctable` is populated with data returned from
      ...
      :func:`gateway.pipeline.loaders.synthetic.make_equity_daily_bar_data`.
...
class WithBcolzFutureDailyBarReader(WithFutureDailyBarData, WithTmpDir):
    ...
    - `cls.bcolz_daily_bar_ctable` is populated with data returned from
      ...
      :func:`gateway.pipeline.loaders.synthetic.make_future_daily_bar_data`.
...
class WithBcolzEquityDailyBarReaderFromCSVs(WithBcolzEquityDailyBarReader):
    """
    GatewayTestCase mixin that provides
...
class WithEquityMinuteBarData(_WithMinuteBarDataBase):
    """
    GatewayTestCase mixin providing cls.equity_minute_bar_days.
    ...
    make_equity_minute_bar_data() -> iterable[(int, pd.DataFrame)]
        ...
        gateway.testing.core.create_minute_bar_data.

    See Also
    --------
    WithEquityDailyBarData
    gateway.testing.create_minute_bar_data
    """
...
class WithFutureMinuteBarData(_WithMinuteBarDataBase):
    """
    GatewayTestCase mixin providing cls.future_minute_bar_days.
    ...
    make_future_minute_bar_data() -> iterable[(int, pd.DataFrame)]
        ...
        :func:`~gateway.testing.create_minute_bar_data`
...
class WithBcolzEquityMinuteBarReader(WithEquityMinuteBarData, WithTmpDir):
    """
    GatewayTestCase mixin providing cls.bcolz_minute_bar_path,
    ...
    - `cls.bcolz_minute_bar_ctable` is populated with data returned from
      ...
      :func:`gateway.pipeline.loaders.synthetic.make_equity_minute_bar_data`.
...
class WithAdjustmentReader(WithBcolzEquityDailyBarReader):
    """
    GatewayTestCase mixin providing cls.adjustment_reader as a class level
    fixture.
    ...
    See Also
    --------
    gateway.testing.MockDailyBarReader
    """
    @classmethod
    def _make_data(cls):
        return None
...
class WithSeededRandomPipelineEngine(WithTradingSessions, WithAssetFinder):
    """
    GatewayTestCase mixin providing class-level fixtures for running pipelines
    ...
    seeded_random_loader : SeededRandomLoader
        ...
        gateway.pipeline.data.testing.TestingDataSet.
    ...
    See Also
    --------
    gateway.pipeline.loaders.synthetic.SeededRandomLoader
    gateway.pipeline.loaders.testing.make_seeded_random_loader
    gateway.pipeline.engine.SimplePipelineEngine
    """
...
class WithDataPortal(WithAdjustmentReader,
                     ...
                     WithBcolzFutureMinuteBarReader):
    """
    GatewayTestCase mixin providing self.data_portal as an instance level
    fixture.
...
class WithResponses(object):
    """
    GatewayTestCase mixin that provides self.responses as an instance
    fixture.
