...
from gateway.pipeline import SimplePipelineEngine, Pipeline, CustomFactor
from gateway.pipeline.common import (
...
from gateway.pipeline.data import DataSet
from gateway.pipeline.data import Column
from gateway.pipeline.loaders.blaze.estimates import (
    BlazeNextEstimatesLoader,
    BlazeNextSplitAdjustedEstimatesLoader,
    BlazePreviousEstimatesLoader,
    BlazePreviousSplitAdjustedEstimatesLoader,
)
from gateway.pipeline.loaders.earnings_estimates import (
...
from gateway.testing.predicates import assert_equal, assert_raises_regex
from gateway.testing.predicates import assert_frame_equal
from gateway.utils.numpy_utils import datetime64ns_dtype
from gateway.utils.numpy_utils import float64_dtype
...
class WithEstimates(WithTradingSessions, WithAdjustmentReader):
    """
    GatewayTestCase mixin providing cls.loader and cls.events as class
    level fixtures.
...
class WithOneDayPipeline(WithEstimates):
    """
    GatewayTestCase mixin providing cls.events as a class level fixture and
...
class PreviousWithOneDayPipeline(WithOneDayPipeline, GatewayTestCase):
    """
...
class NextWithOneDayPipeline(WithOneDayPipeline, GatewayTestCase):
    """
...
class WrongSplitsLoaderDefinition(WithEstimates, GatewayTestCase):
    """
...
class NextEstimate(WithEstimatesTimeZero, GatewayTestCase):
    @classmethod
    def make_loader(cls, events, columns):
        return NextEarningsEstimatesLoader(events, columns)
...
class PreviousEstimate(WithEstimatesTimeZero, GatewayTestCase):
    @classmethod
    def make_loader(cls, events, columns):
        return PreviousEarningsEstimatesLoader(events, columns)
...
class WithEstimateMultipleQuarters(WithEstimates):
    """
    GatewayTestCase mixin providing cls.events, cls.make_expected_out as
...
class NextEstimateMultipleQuarters(
    WithEstimateMultipleQuarters, GatewayTestCase
):
    @classmethod
    def make_loader(cls, events, columns):
        return NextEarningsEstimatesLoader(events, columns)
...
class WithVaryingNumEstimates(WithEstimates):
    """
    GatewayTestCase mixin providing fixtures and a test to ensure that we
...
class WithEstimateWindows(WithEstimates):
    """
    GatewayTestCase mixin providing fixures and a test to test running a
...
class PreviousEstimateWindows(WithEstimateWindows, GatewayTestCase):
    @classmethod
    def make_loader(cls, events, columns):
        return PreviousEarningsEstimatesLoader(events, columns)
...
class NextEstimateWindows(WithEstimateWindows, GatewayTestCase):
    @classmethod
    def make_loader(cls, events, columns):
        return NextEarningsEstimatesLoader(events, columns)
...
class NextWithSplitAdjustedWindows(WithSplitAdjustedWindows, GatewayTestCase):
...
class WithSplitAdjustedMultipleEstimateColumns(WithEstimates):
    """
    GatewayTestCase mixin for having multiple estimate columns that are
...
class PreviousWithSplitAdjustedMultipleEstimateColumns(
    WithSplitAdjustedMultipleEstimateColumns, GatewayTestCase
):
    @classmethod
    def make_loader(cls, events, columns):
        return PreviousSplitAdjustedEarningsEstimatesLoader(
...
class WithAdjustmentBoundaries(WithEstimates):
    """
    GatewayTestCase mixin providing class-level attributes, methods,
...
class QuarterShiftTestCase(GatewayTestCase):
    """
