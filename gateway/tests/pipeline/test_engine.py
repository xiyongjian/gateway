...
from gateway.assets.synthetic import make_rotating_equity_info
from gateway.errors import NoFurtherDataError
from gateway.lib.adjustment import MULTIPLY
from gateway.lib.labelarray import LabelArray
from gateway.pipeline import CustomFactor, Pipeline
from gateway.pipeline.data import Column, DataSet, USEquityPricing
from gateway.pipeline.data.testing import TestingDataSet
from gateway.pipeline.engine import SimplePipelineEngine
from gateway.pipeline.factors import (
    AverageDollarVolume,
    EWMA,
    EWMSTD,
...
from gateway.pipeline.loaders.equity_pricing_loader import (
    USEquityPricingLoader,
)
from gateway.pipeline.loaders.frame import DataFrameLoader
from gateway.pipeline.loaders.synthetic import (
    PrecomputedLoader,
    make_bar_data,
    expected_bar_values_2d,
)
from gateway.pipeline.sentinels import NotSpecified
from gateway.pipeline.term import InputDates
...
from gateway.utils.numpy_utils import bool_dtype, datetime64ns_dtype
...
class ConstantInputTestCase(WithConstantInputs, GatewayTestCase):
...
class FrameInputTestCase(WithTradingEnvironment, GatewayTestCase):
...
class ParameterizedFactorTestCase(WithTradingEnvironment, GatewayTestCase):
...
class PopulateInitialWorkspaceTestCase(WithConstantInputs, GatewayTestCase):
