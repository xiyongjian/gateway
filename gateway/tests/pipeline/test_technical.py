...
from gateway.lib.adjusted_array import AdjustedArray
...
from gateway.testing import check_allclose, parameter_space
from gateway.testing.fixtures import GatewayTestCase
...
class AroonTestCase(GatewayTestCase):
    window_length = 10
    nassets = 5
    dtype = [('down', 'f8'), ('up', 'f8')]
...
class TestFastStochasticOscillator(GatewayTestCase):
    """
...
class IchimokuKinkoHyoTestCase(GatewayTestCase):
...
class TestRateOfChangePercentage(GatewayTestCase):
    @parameterized.expand([
        ('constant', [2.] * 10, 0.0),
        ('step', [2.] + [1.] * 9, -50.0),
...
class TestLinearWeightedMovingAverage(GatewayTestCase):
    def test_wma1(self):
...
class TestTrueRange(GatewayTestCase):
...
class MovingAverageConvergenceDivergenceTestCase(GatewayTestCase):
...
class RSITestCase(GatewayTestCase):
    @parameterized.expand([
...
class AnnualizedVolatilityTestCase(GatewayTestCase):
    """
