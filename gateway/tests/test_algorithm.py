...
from gateway import run_algorithm
from gateway import TradingAlgorithm
from gateway.api import FixedSlippage
from gateway.assets import Equity, Future, Asset
from gateway.assets.continuous_futures import ContinuousFuture
from gateway.assets.synthetic import (
    make_jagged_equity_info,
    make_simple_equity_info,
)
from gateway.data.data_portal import DataPortal
from gateway.data.minute_bars import (
    BcolzMinuteBarReader,
    BcolzMinuteBarWriter,
    US_EQUITIES_MINUTES_PER_DAY,
)
from gateway.data.us_equity_pricing import (
    BcolzDailyBarReader,
    BcolzDailyBarWriter,
)
...
from gateway.finance.commission import PerShare
from gateway.finance.execution import LimitOrder
from gateway.finance.order import ORDER_STATUS
from gateway.finance.trading import SimulationParameters
from gateway.finance.asset_restrictions import (
    Restriction,
...
from gateway.testing import RecordBatchBlotter
from gateway.testing.fixtures import (
    WithDataPortal,
    WithLogger,
    WithSimParams,
...
from gateway.test_algorithms import (
...
from gateway.utils.api_support import GatewayAPI, set_algo_instance
from gateway.utils.calendars import get_calendar, register_calendar
from gateway.utils.context_tricks import CallbackManager, nop_context
import gateway.utils.events
from gateway.utils.events import date_rules, time_rules, Always
import gateway.utils.factory as factory
...
class TestRecordAlgorithm(WithSimParams, WithDataPortal, GatewayTestCase):
...
class TestMiscellaneousAPI(WithLogger,
    ...
    def test_cancel_policy_outside_init(self):
        code = """
from gateway.api import cancel_policy, set_cancel_policy
...
"""
    ...
    def test_cancel_policy_invalid_param(self):
        code = """
from gateway.api import set_cancel_policy
...
"""
    ...
    def test_gateway_api_resolves_dynamically(self):
        # Make a dummy algo.
        ...
        for method in algo.all_api_methods():
            ...
            with GatewayAPI(algo):
                self.assertIs(sentinel, getattr(gateway.api, name)())

    def test_sid_datetime(self):
        algo_text = """
from gateway.api import sid, get_datetime
...
"""
    ...
    def test_datetime_bad_params(self):
        algo_text = """
from gateway.api import get_datetime
...
"""
    ...
    def test_get_environment(self):
        ...
        def initialize(algo):
            self.assertEqual('gateway', algo.get_environment())
...
def log_nyse_close(context, data):
    context.nyse_closes.append(get_datetime())
        ...
        erroring_algotext = dedent(
            """
            from gateway.api import schedule_function
            from gateway.utils.calendars import get_calendar
    ...
    def test_schedule_function_rule_creation(self, mode):
        ...
        algo.schedule_function(nop, time_rule=gateway.utils.events.Never())

        event_rule = algo.event_manager._events[1].rule

        self.assertIsInstance(event_rule, gateway.utils.events.OncePerDay)

        inner_rule = event_rule.rule
        self.assertIsInstance(inner_rule, gateway.utils.events.ComposedRule)

        first = inner_rule.first
        second = inner_rule.second
        composer = inner_rule.composer

        self.assertIsInstance(first, gateway.utils.events.Always)

        if mode == 'daily':
            self.assertIsInstance(second, gateway.utils.events.Always)
        else:
            self.assertIsInstance(second, gateway.utils.events.Never)

        self.assertIs(composer, gateway.utils.events.ComposedRule.lazy_and)
...
class TestTransformAlgorithm(WithLogger,
    ...
    def test_cannot_order_in_before_trading_start(self, order_method, amount):
        algotext = """
from gateway.api import sid
from gateway.api import {order_func}
...
def before_trading_start(context, data):
    ...
    def test_order_on_each_day_of_asset_lifetime(self):
        algo_code = dedent("""
        from gateway.api import sid, schedule_function, date_rules, order
        def initialize(context):
...
class TestBeforeTradingStart(WithDataPortal,
    ...
    def test_data_in_bts_minute(self):
        algo_code = dedent("""
        from gateway.api import record, sid
        def initialize(context):
    ...
    def test_portfolio_bts(self):
        algo_code = dedent("""
        from gateway.api import order, sid, record
...
class TestAlgoScript(WithLogger,
    ...
    def test_fixed_slippage(self):
        ...
        test_algo = TradingAlgorithm(
            script="""
from gateway.api import (slippage,
                         commission,
                         set_slippage,
                         set_commission,
                         order,
...
def handle_data(context, data):
    ...
    def test_incorrectly_set_futures_slippage_model(self):
        code = dedent(
            """
            from gateway.api import set_slippage, slippage
    ...
    def test_order_methods(self):
        ...
        Correct filling of orders is tested in gateway.
        """
        test_algo = TradingAlgorithm(
    ...
    def test_batch_market_order_matches_multiple_manual_orders(self):
        ...
        multi_test_algo = TradingAlgorithm(
            script=dedent("""\
                ...
                from gateway.api import sid, order
        ...
        batch_test_algo = TradingAlgorithm(
            script=dedent("""\
                ...
                from gateway.api import sid, batch_market_order
    ...
    def test_order_dead_asset(self):
        ...
        test_algo = TradingAlgorithm(
            script="""
from gateway.api import order, sid
...
def handle_data(context, data):
    order(sid(0), 10)
        ...
        for order_str in ["order_value", "order_percent"]:
            test_algo = TradingAlgorithm(
                script="""
from gateway.api import order_percent, order_value, sid
...
def handle_data(context, data):
    ...
    def test_schedule_function_time_rule_positionally_misplaced(self):
        ...
        algocode = dedent("""
        from gateway.api import time_rules, schedule_function
...
class TestCapitalChanges(WithLogger,
    ...
    def test_capital_changes_daily_mode(self, change_type, value):
        ...
        algocode = """
from gateway.api import set_slippage, set_commission, slippage, commission, \
...
class TestTradingControls(WithSimParams, WithDataPortal, GatewayTestCase):
    START_DATE = pd.Timestamp('2006-01-03', tz='utc')
    END_DATE = pd.Timestamp('2006-01-06', tz='utc')
...
class TestAccountControls(WithDataPortal, WithSimParams, GatewayTestCase):
    START_DATE = pd.Timestamp('2006-01-03', tz='utc')
    END_DATE = pd.Timestamp('2006-01-06', tz='utc')
...
class TestFutureFlip(WithDataPortal, WithSimParams, GatewayTestCase):
...
class TestFuturesAlgo(WithDataPortal, WithSimParams, GatewayTestCase):
    START_DATE = pd.Timestamp('2016-01-06', tz='utc')
    END_DATE = pd.Timestamp('2016-01-07', tz='utc')
    FUTURE_MINUTE_BAR_START_DATE = pd.Timestamp('2016-01-05', tz='UTC')
...
class TestTradingAlgorithm(WithTradingEnvironment, GatewayTestCase):
...
class TestEquityAutoClose(WithTradingEnvironment, WithTmpDir, GatewayTestCase):
    """
...
class TestOrderAfterDelist(WithTradingEnvironment, GatewayTestCase):
...
class AlgoInputValidationTestCase(WithTradingEnvironment, GatewayTestCase):
...
class TestPanelData(WithTradingEnvironment, GatewayTestCase):
