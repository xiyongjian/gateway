...
"""
Tests for the gateway.assets package
"""
...
from gateway.assets.asset_writer import (
    check_version_info,
    write_version_info,
    _futures_defaults,
    SQLITE_MAX_VARIABLE_NUMBER,
)
from gateway.assets.asset_db_schema import ASSET_DB_VERSION
from gateway.assets.asset_db_migrations import (
    downgrade
)
...
from gateway.utils.range import range
...
class TestFuture(WithAssetFinder, GatewayTestCase):
    @classmethod
...
class AssetFinderTestCase(WithTradingCalendars, GatewayTestCase):
...
class TestAssetDBVersioning(GatewayTestCase):
...
class TestVectorizedSymbolLookup(WithAssetFinder, GatewayTestCase):
...
class TestAssetFinderPreprocessors(WithTmpDir, GatewayTestCase):
