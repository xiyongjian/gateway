...
def window_specialization(typename):
    ...
    return Extension(
        'gateway.lib._{name}window'.format(name=typename),
        ['gateway/lib/_{name}window.pyx'.format(name=typename)],
        depends=['gateway/lib/_windowtemplate.pxi'],
    )


ext_modules = [
    Extension('gateway.assets._assets', ['gateway/assets/_assets.pyx']),
    Extension('gateway.assets.continuous_futures',
              ['gateway/assets/continuous_futures.pyx']),
    Extension('gateway.lib.adjustment', ['gateway/lib/adjustment.pyx']),
    Extension('gateway.lib._factorize', ['gateway/lib/_factorize.pyx']),
    window_specialization('float64'),
    window_specialization('int64'),
    window_specialization('int64'),
    window_specialization('uint8'),
    window_specialization('label'),
    Extension('gateway.lib.rank', ['gateway/lib/rank.pyx']),
    Extension('gateway.data._equities', ['gateway/data/_equities.pyx']),
    Extension('gateway.data._adjustments', ['gateway/data/_adjustments.pyx']),
    Extension('gateway._protocol', ['gateway/_protocol.pyx']),
    Extension('gateway.gens.sim_engine', ['gateway/gens/sim_engine.pyx']),
    Extension(
        'gateway.data._minute_bar_internal',
        ['gateway/data/_minute_bar_internal.pyx']
    ),
    Extension(
        'gateway.utils.calendars._calendar_helpers',
        ['gateway/utils/calendars/_calendar_helpers.pyx']
    ),
    Extension(
        'gateway.data._resample',
        ['gateway/data/_resample.pyx']
    ),
    Extension(
        'gateway.pipeline.loaders.blaze._core',
        ['gateway/pipeline/loaders/blaze/_core.pyx'],
        depends=['gateway/lib/adjustment.pxd'],
    ),
]
...
def read_requirements(path,
                      ...
                      filter_names=None):
    """
    Read a requirements.txt file, expressed as a path relative to Gateway root.
...
setup(
    ...
    packages=find_packages(include=['gateway', 'gateway.*']),
    ext_modules=ext_modules,
    include_package_data=True,
    package_data={root.replace(os.sep, '.'):
                  ['*.pyi', '*.pyx', '*.pxi', '*.pxd']
                  for root, dirnames, filenames in os.walk('gateway')
