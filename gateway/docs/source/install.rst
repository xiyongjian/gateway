...
Installing Gateway via ``pip`` is slightly more involved than the average
Python package.
...
If you use Python for anything other than Gateway, we **strongly** recommend
...
3.5, but Gateway only currently supports 3.4), and `ta-lib
<https://aur.archlinux.org/packages/ta-lib/>`_, an optional Gateway dependency.
...
For windows, the easiest and best supported way to install gateway is to use
:ref:`Conda <conda>`.
...
``scipy``.  This means that ``conda`` can install Gateway and its dependencies
without requiring the use of a second tool to acquire Gateway's non-Python
dependencies.

For instructions on how to install ``conda``, see the `Conda Installation
Documentation <http://conda.pydata.org/docs/download.html>`_

Once ``conda`` has been set up you can install Gateway from our ``Quantopian``
channel:
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
It is recommended to install Gateway in an isolated ``conda`` environment. 
Installing Gateway in ``conda`` environments will not interfere your default 
...
.. code-block:: bash
    ...
    $ conda create -n env_gateway python=2.7

- Python 3.5:

.. code-block:: bash

    $ conda create -n env_gateway python=3.5


Now you have set up an isolated environment called ``env_gateway``, a sandbox-like
structure to install Gateway. Then you should activate the conda environment 
...
You can install Gateway by running

.. code-block:: bash

    (env_gateway) $ conda install -c Quantopian gateway

To deactivate the ``conda`` environment:

.. code-block:: bash

    (env_gateway) $ source deactivate
