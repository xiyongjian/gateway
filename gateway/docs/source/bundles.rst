...
Gateway comes with a few bundles by default as well as the ability to register
...
- ``quandl`` (provided by gateway)
- ``quantopian-quandl`` (provided by gateway)
...
standard location that gateway can find. By default the location where ingested
data will be written is ``$GATEWAY_ROOT/data/<bundle>`` where by default
``GATEWAY_ROOT=~/.gateway``. The ingestion step may take some time as it could
...
.. code-block:: bash
   ...
   $ QUANDL_API_KEY=<yourkey> gateway ingest [-b <bundle>]
...
of ``$GATEWAY_ROOT/data/<bundle>`` which is named with the current date. This
...
.. code-block:: bash
   ...
   $ gateway clean [-b <bundle>] --before <date>

   # clean everything newer than <date>
   $ gateway clean [-b <bundle>] --after <date>

   # keep everything in the range of [before, after] and delete the rest
   $ gateway clean [-b <bundle>] --before <date> --after <after>

   # clean all but the last <int> runs
   $ gateway clean [-b <bundle>] --keep-last <int>
...
.. code-block:: bash
   ...
   $ gateway run --bundle <bundle> --algofile algo.py ...
...
By default gateway comes with the ``quandl`` data bundle which uses quandl's
...
.. code-block:: bash
   ...
   $ QUANDL_API_KEY=<api-key> gateway ingest -b quandl
...
gateway. To add a new bundle, one must implement an ``ingest`` function.

The ``ingest`` function is responsible for loading the data into memory and
passing it to a set of writer objects provided by gateway to convert the data to
gateway's internal format. The ingest function may work by downloading data from
...
``asset_db_writer`` is an instance of :class:`~gateway.assets.AssetDBWriter`.
...
:meth:`~gateway.assets.AssetDBWriter.write` with dataframes for the various
...
:class:`~gateway.data.minute_bars.BcolzMinuteBarWriter`. This writer is used to
convert data to gateway's internal bcolz format to later be read by a
:class:`~gateway.data.minute_bars.BcolzMinuteBarReader`. If minute data is
provided, users should call
:meth:`~gateway.data.minute_bars.BcolzMinuteBarWriter.write` with an iterable of
...
empty iterator to :meth:`~gateway.data.minute_bars.BcolzMinuteBarWriter.write`
to signal that there is no minutely data.

.. note::

   The data passed to
   :meth:`~gateway.data.minute_bars.BcolzMinuteBarWriter.write` may be a lazy
...
:class:`~gateway.data.us_equity_pricing.BcolzDailyBarWriter`. This writer is
used to convert data into gateway's internal bcolz format to later be read by a
:class:`~gateway.data.us_equity_pricing.BcolzDailyBarReader`. If daily data is
provided, users should call
:meth:`~gateway.data.minute_bars.BcolzDailyBarWriter.write` with an iterable of
(sid dataframe) tuples. The ``show_progress`` argument should also be forwarded
...
iterable to :meth:`~gateway.data.minute_bars.BcolzMinuteBarWriter.write` to
...
:class:`~gateway.data.us_equity_pricing.SQLiteAdjustmentWriter`. This writer is
used to store splits, mergers, dividends, and stock dividends. The data should
be provided as dataframes and passed to
:meth:`~gateway.data.us_equity_pricing.SQLiteAdjustmentWriter.write`. Each of
...
:class:`gateway.utils.calendars.TradingCalendar`. The calendar is provided to
...
``cache`` is an instance of :class:`~gateway.utils.cache.dataframe_cache`. This
...
:class:`~gateway.utils.cli.maybe_show_progress`. This argument should always be
...
written. ``output_dir`` will be some subdirectory of ``$GATEWAY_ROOT`` and will
...
Gateway provides a bundle called ``csvdir``, which allows users to ingest data
...
purposes in ``gateway/tests/resources/csvdir_samples``.
...
``~/.gateway/extension.py`` and import the csvdir bundle, along with ``pandas``.

.. code-block:: python

	 import pandas as pd
	 
	 from gateway.data.bundles import register
	 from gateway.data.bundles.csvdir import csvdir_equities
...
.. code-block:: bash
	 ...
	 $ gateway ingest -b custom-csvdir-bundle
	 ...
	 $ CSVDIR=/path/to/your/csvs gateway ingest -b custom-csvdir-bundle


If you would like to use equities that are not in the NYSE calendar, or the existing gateway calendars,
