Gateway Beginner Tutorial
-------------------------

Basics
~~~~~~

Gateway is an open-source algorithmic trading simulator written in
Python.

The source can be found at: https://github.com/quantopian/gateway
...
-  Developed and continuously updated by
   ...
   easy-to-use web-interface to Gateway, 10 years of minute-resolution
   historical US stock data, and live-trading capabilities. This
   tutorial is directed at users wishing to use Gateway without using
   Quantopian. If you instead want to get started on Quantopian, see
   `here <https://www.quantopian.com/faq#get-started>`__.

This tutorial assumes that you have gateway correctly installed, see the
`installation
instructions <https://github.com/quantopian/gateway#installation>`__ if
you haven't set up gateway yet.

Every ``gateway`` algorithm consists of two functions you have to
define:

* ``initialize(context)``
* ``handle_data(context, data)``

Before the start of the algorithm, ``gateway`` calls the
...
After the algorithm has been initialized, ``gateway`` calls the
...
.. code-block:: python
   ...
   from gateway.examples import buyapple
   buyapple??


.. code-block:: python

   from gateway.api import order, record, symbol
...
``gateway.api``. Here we are using :func:`~gateway.api.order()` which takes two
arguments: a security object, and a number specifying how many stocks you would
like to order (if negative, :func:`~gateway.api.order()` will sell/short
...
Finally, the :func:`~gateway.api.record` function allows you to save the value
...
with :func:`~gateway.api.record` under the name you provided (we will see this
...
To now test this algorithm on financial data, ``gateway`` provides three
...
After you installed gateway you should be able to execute the following
...
``buyapple_out.pickle`` we would call ``gateway run`` as follows:

.. code-block:: python

    gateway run -f ../../gateway/examples/buyapple.py --start 2016-1-1 --end 2018-1-1 -o buyapple_out.pickle
...
After each call to ``handle_data()`` we instruct ``gateway`` to order 10
stocks of AAPL. After the call of the ``order()`` function, ``gateway``
enters the ordered stock and amount in the order book. After the
``handle_data()`` function has finished, ``gateway`` looks for any open
...
slippage model that ``gateway`` uses, see the `Quantopian
...
rows. Note that ``gateway`` makes heavy usage of ``pandas``, especially
...
quantitative researchers ``gateway`` provides an easy way to run your
algorithm inside the Notebook without requiring you to use the CLI.

To use it you have to write your algorithm in a cell and let ``gateway``
know that it is supposed to run this algorithm. This is done via the
``%%gateway`` IPython magic command that is available after you
``import gateway`` from within the IPython Notebook. This magic takes
...
execute the following cell after importing ``gateway`` to register the
magic.
...
.. code-block:: python
   ...
   %%gateway --start 2016-1-1 --end 2018-1-1
   from gateway.api import symbol, order, record
...
.. code-block:: python
   ...
   %%gateway --start 2014-1-1 --end 2018-1-1 -o dma.pickle
...
architecture, API, and features of ``gateway``. For next steps, check
out some of the
`examples <https://github.com/quantopian/gateway/tree/master/gateway/examples>`__.

Feel free to ask questions on `our mailing
list <https://groups.google.com/forum/#!forum/gateway>`__, report
problems on our `GitHub issue
tracker <https://github.com/quantopian/gateway/issues?state=open>`__,
`get
involved <https://github.com/quantopian/gateway/wiki/Contribution-Requests>`__,
