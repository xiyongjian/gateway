...
When we are ready to ship a new release of gateway, edit the :doc:`releases`
...
example, we generate stub files for the :mod:`~gateway.api` namespace, since that
...
.. note::
   ...
   ``gateway.api``, this is done in a header string in the
   ``gen_type_stubs.py`` script mentioned above.  If new classes are added as
   parameters or return types of ``gateway.api`` functions, then new imports
...
Next, click the "Draft a new release" button on the `gateway releases page
<https://github.com/quantopian/gateway/releases>`__.  For the new release,
...
from the gateway root. This will create a gzipped tarball that includes all the
python, cython, and miscellaneous files needed to install gateway. To test that
...
.. code-block:: bash
   ...
   $ pip install <gateway-root>/dist/gateway-<major>.<minor>.<micro>.tar.gz
   $ python -c 'import gateway;print(gateway.__version__)'
...
This will upload gateway to the pypi test server. To test installing from pypi,
create a new virtualenv, ``cd`` into a clean directory and then run:

.. code-block:: bash

   $ pip install --extra-index-url https://testpypi.python.org/pypi gateway
   $ python -c 'import gateway;print(gateway.__version__)'
...
Because gateway now supports multiple versions of numpy, we're not building
...
To update `gateway.io <http://www.gateway.io/index.html>`__, checkout the
latest master and run:

.. code-block:: python

    python <gateway_root>/docs/deploy.py

This will build the documentation, checkout a fresh copy of the ``gh-pages``
git branch, and copy the built docs into the gateway root.
...
.. code-block:: bash
   ...
   $ git add .
   $ git commit -m "DOC: update gateway.io"
   $ git push origin gh-pages

`gateway.io <http://www.gateway.io/index.html>`__ will update in a few moments.

Uploading conda packages
~~~~~~~~~~~~~~~~~~~~~~~~

Travis and AppVeyor build gateway conda packages for us.  Once they have built
...
To build the conda packages for gateway locally, run:
