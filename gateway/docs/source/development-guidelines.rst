...
======================
This page is intended for developers of Gateway, people who want to contribute to the Gateway codebase or documentation, or people who want to install from source and make local changes to their copy of Gateway.
...
First, you'll need to clone Gateway by running:

.. code-block:: bash

   $ git clone git@github.com:your-github-username/gateway.git
...
.. code-block:: bash
   ...
   # where gateway is the name of your virtualenv
   $ deactivate gateway
   $ workon gateway
...
If you want to work with gateway using a `Docker`__ container, you'll need to build the ``Dockerfile`` in the Gateway root directory, and then build ``Dockerfile-dev``. Instructions for building both containers can be found in ``Dockerfile`` and ``Dockerfile-dev``, respectively.
...
We use `flake8`__ for checking style requirements and `nosetests`__ to run Gateway tests. Our `continuous integration`__ tools will run these commands.
...
__ https://travis-ci.org/quantopian/gateway
__ https://ci.appveyor.com/project/quantopian/gateway


Packaging
---------
To learn about how we build Gateway conda packages, you can read `this`__ section in our release process notes.
...
If you'd like to contribute to the documentation on gateway.io, you can navigate to ``docs/source/`` where each `reStructuredText`__ (``.rst``) file is a separate section there. To add a section, create a new file called ``some-descriptive-name.rst`` and add ``some-descriptive-name`` to ``appendix.rst``. To edit a section, simply open up one of the existing files, make your changes, and save them.

__ https://en.wikipedia.org/wiki/ReStructuredText

We use `Sphinx`__ to generate documentation for Gateway, which you will need to install by running:
...
.. code-block:: bash
   ...
   # assuming you're in the Gateway root directory
   $ cd docs
   $ make html
...
.. code-block:: text
   ...
   BLD: change related to building Gateway
   BUG: bug fix
   ...
   REL: related to releasing Gateway
