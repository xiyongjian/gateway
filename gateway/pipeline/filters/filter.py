...
from gateway.lib.rank import is_missing, grouped_masked_is_maximal
...
class Filter(RestrictedDTypeMixin, ComputableTerm):
    ...
    requested computation. For example, :meth:`gateway.pipeline.Factor.top`
    ...
    operators (``<``, ``<=``, ``!=``, ``eq``, ``>``, ``>=``) of
    :class:`~gateway.pipeline.Factor`. For example, a natural way to construct
    ...
    20.0::
        ...
        >>> from gateway.pipeline.factors import VWAP
...
class PercentileFilter(SingleInputMixin, Filter):
    ...
    Parameters
    ----------
    factor : gateway.pipeline.factor.Factor
...
class CustomFilter(PositiveWindowLengthMixin, CustomTermMixin, Filter):
    ...
    :class:`~gateway.pipeline.factors.factor.CustomFactor` for more details on
    implementing a custom ``compute`` method.

    See Also
    --------
    gateway.pipeline.factors.factor.CustomFactor
    """
    def _validate(self):
        try:
