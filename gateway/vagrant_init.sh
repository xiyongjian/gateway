...
# set up everything necessary to use Gateway.
...
# from the root of the gateway github checkout.
# This will put the gateway code in the
...
echo "Installing gateway python dependencies..." | tee -a "$VAGRANT_LOG"
/vagrant/etc/ordered_pip.sh /vagrant/etc/requirements.txt 2>&1 | tee -a "$VAGRANT_LOG"
echo "Installing gateway extra python dependencies..." | tee -a "$VAGRANT_LOG"
pip install -r /vagrant/etc/requirements_dev.txt -r /vagrant/etc/requirements_blaze.txt 2>&1 | tee -a "$VAGRANT_LOG"
echo "Installing gateway package itself..." | tee -a "$VAGRANT_LOG"
...
echo "Finished!  gateway repo is in '/vagrant'." | tee -a "$VAGRANT_LOG"
