...
from gateway._protocol import handle_non_market_minutes
from gateway.assets.synthetic import make_simple_equity_info
from gateway.data.data_portal import DataPortal
from gateway.data.resample import minute_panel_to_session_panel
from gateway.data.us_equity_pricing import PanelBarReader
...
from gateway.finance.trading import TradingEnvironment
from gateway.finance.blotter import Blotter
from gateway.finance.controls import (
    LongOnly,
    MaxOrderCount,
    MaxOrderSize,
    MaxPositionSize,
...
from gateway.finance.execution import (
    LimitOrder,
    MarketOrder,
    StopLimitOrder,
    StopOrder,
)
from gateway.finance.performance import PerformanceTracker
from gateway.finance.asset_restrictions import Restrictions
from gateway.finance.cancel_policy import NeverCancel, CancelPolicy
...
from gateway.gens.tradesimulation import AlgorithmSimulator
from gateway.pipeline import Pipeline
from gateway.pipeline.engine import (
    ExplodingPipelineEngine,
    SimplePipelineEngine,
)
from gateway.utils.api_support import (
    api_method,
    require_initialized,
    require_not_initialized,
    GatewayAPI,
    disallowed_in_before_trading_start)
from gateway.utils.input_validation import (
    coerce_string,
    ensure_upper_case,
    error_keywords,
    expect_dtypes,
...
from gateway.utils.numpy_utils import int64_dtype
from gateway.utils.calendars.trading_calendar import days_at_time
from gateway.utils.cache import ExpiringCache
from gateway.utils.calendars import get_calendar
from gateway.utils.pandas_utils import clear_dataframe_indexer_caches

import gateway.utils.events
from gateway.utils.events import (
    EventManager,
    make_eventrule,
    date_rules,
    time_rules,
...
from gateway.utils.factory import create_simulation_parameters
from gateway.utils.math_utils import (
    tolerant_equals,
    round_if_near_integer,
)
from gateway.utils.preprocess import preprocess
...
class TradingAlgorithm(object):
    ...
    *args, **kwargs
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._platform = kwargs.pop('platform', 'gateway')
        ...
        if not self.blotter:
            self.blotter = Blotter(
                ...
                # Default to NeverCancel in gateway
        ...
        self.event_manager.add_event(
            gateway.utils.events.Event(
                gateway.utils.events.Always(),
    ...
    def initialize(self, *args, **kwargs):
        """
        Call self._initialize with `self` made available to Gateway API
        functions.
        """
        with GatewayAPI(self):
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def run(self, data=None, overwrite_sim_params=True):
        ...
        # Create gateway and loop through simulated_trading.
        ...
        field : {'platform', 'arena', 'data_frequency',
            ...
            The field to query. The options have the following meanings:
              ...
              platform : str
                  ...
                  will be the string 'gateway'. This can allow algorithms to
    ...
    def fetch_csv(self,
        ...
        **kwargs
        ...
        Returns
        -------
        csv_data_source : gateway.sources.requests_csv.PandasRequestsCSV
    ...
    def add_event(self, rule=None, callback=None):
        ...
        """
        self.event_manager.add_event(
            gateway.utils.events.Event(rule, callback),
        )
    ...
    def schedule_function(self,
        ...
        See Also
        --------
        :class:`gateway.api.date_rules`
        :class:`gateway.api.time_rules`
        """
    ...
    def symbol(self, symbol_str):
        ...
        See Also
        --------
        :func:`gateway.api.set_symbol_lookup_date`
        """
    ...
    def order(self,
        ...
        See Also
        --------
        :class:`gateway.finance.execution.ExecutionStyle`
        :func:`gateway.api.order_value`
        :func:`gateway.api.order_percent`
        """
    ...
    def _calculate_order(self, asset, amount,
        ...
        # Raises a GatewayError if invalid parameters are detected.
    ...
    def order_value(self,
        ...
        Notes
        -----
        See :func:`gateway.api.order` for more information about
        ``limit_price``, ``stop_price``, and ``style``

        See Also
        --------
        :class:`gateway.finance.execution.ExecutionStyle`
        :func:`gateway.api.order`
        :func:`gateway.api.order_percent`
        """
        if not self._can_order_asset(asset):
            return None
    ...
    def set_slippage(self, us_equities=None, us_futures=None):
        ...
        See Also
        --------
        :class:`gateway.finance.slippage.SlippageModel`
        """
        if self.initialized:
    ...
    def set_commission(self, us_equities=None, us_futures=None):
        ...
        See Also
        --------
        :class:`gateway.finance.commission.PerShare`
        :class:`gateway.finance.commission.PerTrade`
        :class:`gateway.finance.commission.PerDollar`
        """
        if self.initialized:
    ...
    def set_cancel_policy(self, cancel_policy):
        ...
        See Also
        --------
        :class:`gateway.api.EODCancel`
        :class:`gateway.api.NeverCancel`
        """
    ...
    def order_target(self,
        ...
        :class:`gateway.finance.execution.ExecutionStyle`
        :func:`gateway.api.order`
        :func:`gateway.api.order_target_percent`
        :func:`gateway.api.order_target_value`
        """
        if not self._can_order_asset(asset):
            return None
    ...
    def cancel_order(self, order_param):
        ...
        if isinstance(order_param, gateway.protocol.Order):
    ...
    def history(self, bar_count, frequency, field, ffill=True):
        ...
        warnings.warn(
            ...
            category=GatewayDeprecationWarning,
            stacklevel=4
        )
    ...
    def set_do_not_order_list(self, restricted_list, on_error='fail'):
        ...
        if isinstance(restricted_list, SecurityList):
            ...
            warnings.warn(
                ...
                "Create a gateway.finance.asset_restrictions."
    ...
    def set_asset_restrictions(self, restrictions, on_error='fail'):
        ...
        See Also
        --------
        gateway.finance.asset_restrictions.Restrictions
        """
    ...
    def attach_pipeline(self, pipeline, name, chunks=None):
        ...
        See Also
        --------
        :func:`gateway.api.pipeline_output`
        """
        if chunks is None:
    ...
    def pipeline_output(self, name):
        ...
        See Also
        --------
        :func:`gateway.api.attach_pipeline`
        :meth:`gateway.pipeline.engine.PipelineEngine.run_pipeline`
        """
        try:
