...
from gateway.utils.algo_instance import get_algo_instance, set_algo_instance


class GatewayAPI(object):
    """
    Context manager for making an algorithm instance available to gateway API
...
def api_method(f):
    ...
    # function (wrapped) to gateway.api
    @wraps(f)
    def wrapped(*args, **kwargs):
        ...
        if algo_instance is None:
            raise RuntimeError(
                'gateway api method %s must be called during a simulation.'
                % f.__name__
            )
        return getattr(algo_instance, f.__name__)(*args, **kwargs)
    # Add functor to gateway.api
    setattr(gateway.api, f.__name__, wrapped)
    gateway.api.__all__.append(f.__name__)
