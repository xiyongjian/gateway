...
from gateway.assets import AssetFinder, AssetDBWriter
...
from gateway.data.loader import get_benchmark_filename, INDEX_MAPPING
...
from gateway.lib.labelarray import LabelArray
from gateway.pipeline.data import USEquityPricing
from gateway.pipeline.engine import SimplePipelineEngine
from gateway.pipeline.factors import CustomFactor
from gateway.pipeline.loaders.testing import make_seeded_random_loader
from gateway.utils import security_list
from gateway.utils.calendars import get_calendar
from gateway.utils.input_validation import expect_dimensions
from gateway.utils.numpy_utils import as_column, isnat
from gateway.utils.pandas_utils import timedelta_to_integral_seconds
from gateway.utils.paths import ensure_directory
from gateway.utils.sentinel import sentinel
...
def drain_gateway(test, gateway):
    output = []
    transaction_count = 0
    msg_counter = 0
...
def assert_single_position(test, gateway):

    output, transaction_count = drain_gateway(test, gateway)

    if 'expected_transactions' in test.gateway_test_config:
        test.assertEqual(
            test.gateway_test_config['expected_transactions'],
            transaction_count
        )
    else:
        test.assertEqual(
            test.gateway_test_config['order_count'],
            transaction_count
        )
    ...
    sid = test.gateway_test_config['sid']
    test.assertEqual(
...
def all_pairs_matching_predicate(values, pred):
    ...
    Examples
    --------
    >>> from gateway.testing import all_pairs_matching_predicate
...
def subtest(iterator, *_names):
    ...
    Consider using ``gateway.testing.parameter_space`` when subtests
    ...
    * Have a large parameter space we are testing
    ...
    See Also
    --------
    gateway.testing.parameter_space
    """
    def dec(f):
        @wraps(f)
...
def test_resource_path(*path_parts):
    return os.path.join(gateway_git_root, 'tests', 'resources', *path_parts)
