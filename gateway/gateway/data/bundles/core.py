...
from gateway.assets import AssetDBWriter, AssetFinder, ASSET_DB_VERSION
from gateway.assets.asset_db_migrations import downgrade
from gateway.utils.cache import (
    dataframe_cache,
    working_dir,
    working_file,
)
from gateway.utils.compat import mappingproxy
from gateway.utils.input_validation import ensure_timestamp, optionally
    ...
    def register(name,
        ...
        f : callable
            The ingest function. This function will be passed:
              ...
              calendar : gateway.utils.calendars.TradingCalendar
        ...
        # `register` is called at module scope in gateway, and creating a
        # calendar currently takes between 0.5 and 1 seconds, which causes a
        # noticeable delay on the gateway CLI.
    ...
    def most_recent_data(bundle_name, timestamp, environ=None):
        ...
        try:
            ...
            raise ValueError(
                ...
                'maybe you need to run: $ gateway ingest -b {bundle}'.format(
                    bundle=bundle_name,
                    timestamp=timestamp,
                ),
            )
